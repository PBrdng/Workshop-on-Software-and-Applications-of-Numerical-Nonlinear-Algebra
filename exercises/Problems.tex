...
\begin{itemize}
\item if $C=\mathcal{V}(f(x,y))$ is an irreducible curve, and $L_t$ a generic parallel family of lines, then the sum of the points $C \cap L_t$ moves linearly in $t$. \\ (Hint: assume $L_t$ are the lines $\mathcal{V}(x-t)$, and parametrize them)
...
\item Given a surface $X \subset \mathbb{P}^3$, the {\textit{flecnodal surface}} $\mathcal{F}(X)$ is the union of all lines $L$ with contact order $4$ at a point of $X$. It is one of five {\textit{event surfaces}} described in \href{https://arxiv.org/pdf/1707.01877.pdf}{ ``Changing views on curves and surfaces"} by Kohn, Sturmfels, and Trager.
\begin{itemize}
...
\item Consider the quintic surface $$f = x_1^5+x_2^5+x_3^5+x_4^5+(x_1+x_2+x_3+x_4)^5+x_1x_2x_3x_4(x_1+x_2+x_3+x_5)$$
in $\mathbb{P}^3$. Parametrize lines by 
$$z(t) = (q_{1,2}:tq_{1,2}:tq_{1,3}-q_{2,3}:tq_{1,4}-q_{2,4})$$
(here $\{q_{1,2},q_{1,3},q_{1,4},q_{2,3},q_{2,4},q_{3,4}\}$ are Pl\"ucker coordinates).

Compute a witness set for the Flecnodal surface of $X=\mathcal{V}(f)$ in the coordinates $q$ (remember to include the Pl\"ucker relation $q_{1,2}q_{3,4}-q_{1,3}q_{2,4}+q_{1,4}q_{2,3}=0$.) What is its degree?
\end{itemize}
